<template>
    <div class="gr_container  bg_primary">
        <GameSubmenu />
        <div class="gr_game-my-classic">
            <div class="gr_old-tab" v-if="+MY_menuIndex === 2">
                <div
...
<script>
    ...
    import GameSubmenu from '@C/games/MY/components/GameSubmenu'
    export default {
        name: 'GameMY',
        data() {
            return {
        ...
        beforeMount() {
            ...
            if (this.MY_gameSubMenu.length) {
                let MY_gameSubMenu = {}
                const data = this.MY_gameSubMenu[0]
                MY_gameSubMenu[data.methodId] = true
        ...
        computed: {
            ...mapGetters([
                ...
                'MY_jsPrizePoint',
                'MY_gameSubMenu',
                'MY_methodSelect', // method 是否選取的資料
                'MY_methodIsSelect', // 塞選好的 method 列表
                'MY_gameSelectData', // 選球資料
                'MY_gameSelectDataArr'
            ]),
            numLen() {
                return this.MY_menuIndex + 1
            },
            activeData() {
                let obj = {}
        ...
        methods: {
            ...
            handleOrder(codes, bool) {
                if (this.MY_stake) {
                    const data = {...this.MY_gameSelectData}
                    this.MY_methodIsSelect.forEach((methodId) => {
                        if (bool) {
                            codes.forEach((code) => {
                                data[`${methodId}_${code}`] = {
                                    methodId,
                                    codes: code,
                                    money: Math.max(+this.MY_money, this.MY_stake),
            ...
            hprize(id) {
                if (this.MY_jsPrizePoint) {
                    const { hprize, autoOdds } = this.MY_jsPrizePoint[id][0]

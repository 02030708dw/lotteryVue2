<template>
    <div :class="[
        ...
        @click.stop>
        <div class="gr_game-la-history__button-group">
            <a
        ...
        <div class="gr_game-la-history__title">
            <!-- '历史奖号' -->
            {{$t('common_007')}}
        </div>
        <div class="gr_game-la-history__scroll-area">
            <div class="gr_game-la-history__list" v-for="(item, index) in LA_historyBall.slice(0, 10)" :key="index">
            ...
            <div class="gr_game-la-history__list is_empty" v-if="!LA_historyBall || LA_historyBall.length === 0" >
                <!-- '沒有奖期' -->
                {{$t('common_011')}}
            </div>
        </div>
...
export default {
    ...
    computed: {
        ...
        fillterLA_historyBall() {
            return this.LA_historyBall.map((e) => {
                return e.split(',')
            })
        }
        // 判斷是否為自開彩

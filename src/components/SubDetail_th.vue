    ...
    <div class="gr_page-detail__container gr_page-detail__body">
        <div>
            ...
            <dl>
                ...
                <dd>{{parent.isOfficial(parent.info.lottery_id) ? transBelongDate(parent.info.belong_date) : parent.info.issue}}</dd>
            </dl>
...
export default {
    ...
    data() {
        return {
            // 头奖/前三/后三/二星
            bingoTitleArr: ['thai_012', '4d_o_026', '4d_o_017', '4d_o_029'],
            keyObj: {
                '1D': 'thai_011',
                '2D': 'thai_010',
                '3D': 'thai_009'
    ...
    computed: {
        ...
        bingoCode() {
            if (this.getData.bingo_code === '') {
                return []
            }
            return this.getData.bingo_code.split(',').reduce((formatArr, code, codeIndex) => {

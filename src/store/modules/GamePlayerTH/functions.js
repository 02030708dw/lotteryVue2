...
export const reCheckJsDynamicConfig = (jsDynamicConfig, params, num, {rootGetters, commit, dispatch}) => {
    ...
    const currentGame = rootGetters.lotteryList2[rootGetters.TH_lotteryId]
    ...
    try {
        ...
        if (diff !== 1 && diff !== -9) {
            ...
            commit(_M.SET_GAME_PLAY_DYNAMIC_TH_DATA, config)
        }
    } catch (err) {
...
export const handleStopBetNumber = async(params, {state, commit, rootGetters}) => {
    const fetchData = await handleAjax(API.stopBetting, params, rootGetters)
    const { stop_bet_number } = fetchData.data
...
export const handlePrizeData = (floatPrize, state, commit) => {
    if (floatPrize) {
        ...
        commit(_M.SET_TH_FLOAT_PRIZE, { prizeBar, keepPoint, jsPrizePoint })
    }
}
/**
 * 判斷是否有獎期
...
export const handleJsDynamicConfig = (jsDynamicConfig, params, {commit, rootGetters, dispatch}) => {
    ...
    commit(_M.SET_SUBMIT_TH_DATA, { ltIssueStart })
    commit(_M.SET_FLIP_TIMEER_TH_DATA, timeData)

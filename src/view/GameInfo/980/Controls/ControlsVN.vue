<template>
    <div class='gr_container bg_primary gr_page__controls u_clearfix'>
        ...
        <div class="u_d--inline-block">
            <!-- '所有彩种' -->
            <el-select v-if="!isW88" :class='className' :className='className' v-model="lotteryId" clearable :placeholder="$t(games)" :listGroup='areaList' filterable>
                <el-option-group v-for="(item, index) in areaList" :key="`${item.title_key}${index}`" :label="$t(item.title_key)">

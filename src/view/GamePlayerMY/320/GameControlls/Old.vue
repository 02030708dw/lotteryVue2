...
<template>
    <div class="game_controlls container">
        ...
        <ul v-else>
            ...
            <li>
                <button class="c_button--secondary" v-show="(!MY_gameSelectDataArr.length || isBlocked) && boxLength > 0" @click.stop="handleGameBox">
            ...
            <li>
                <button class="c_button--secondary" v-show="MY_gameSelectDataArr.length && !isBlocked" @click.stop="add">
            ...
            <li>
                <button
                    ...
                    @click="(!isFastDisabled && MY_gameSelectDataArr.length) && fast()"
                >
					<!-- {{$t("直接投注")}} : '本期停售'-->
...
export default {
    ...
    methods: {
        ...
        //確認投注
        async confirm() {
            const errorCode = await this[_M.GAME_ORDER_MY_SUBMIT]({instance: this})
            if (errorCode === 0) {
                this.$root.$emit('back')
            }
        },
        ...
        fast() {
            ...
            this[_M.GAME_ORDER_MY_SUBMIT]({instance: this, fast: true})
        },
        reset() {
    ...
    computed: {
        ...
        isFastDisabled() {
            return this.MY_gameSelectDataArr.length ? null : 'c_button--disabled'
        }
    }
}
</script>
...
export default {
    ...
    methods: {
        createPopper() {
            if (!this.popperElm) return //投注頁參數說明
            if (this.$root._route.name === 'GamePlayer') return //此方法造成投注頁dropdown select跑版，因此在投注頁不執行
        ...
        resetTransformOrigin() {
            ...
            this.popperJS._popper.style.transformOrigin = ['top', 'bottom'].indexOf(
                    placement
                ) > -1 ?
                `center ${origin}` :
                `${origin} center`

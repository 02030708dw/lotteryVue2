...
export default {
    ...
    methods: {
        ...
        drawCalculator(value, fn) {
            ...
            const close = new Button({ id: 'calculator', key: 'close', x: 374, y: 38, anchor: {x: 0.5, y: 0.5}, p }, {
            ...
            // 數字框
            const valueBg = new Graphics(p, {name: 'valueBg', x: 345, y: 64})
            const style = {
                fontFamily: 'Arial',
                fontSize: 34,
                fontWeight: 'bold',
            ...
            // 數字
            const text = new Text({text: value, p: valueBg, style: style2, anchor: {x: 1, y: 0}})
            // 按鈕資料
            const buttonArr = [
                [7, 8, 9],
                [4, 5, 6],
            ...
            buttonArr.forEach((arr, i) => {
                ...
                arr.forEach((text, j) => {
                    ...
                    const key = new Text({text, p: btn, style: ['C', 'OK'].indexOf(text) >= 0 ? {...style, fontSize: 28} : style, x: 60, y: 32})

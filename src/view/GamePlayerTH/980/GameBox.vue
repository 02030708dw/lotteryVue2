    ...
    <div class="gr_container  bg_accent  u_clearfix">
        ...
        <div class="content bg_secondary_light">
            <div class="content_inner">
                <div class="list u_clearfix" v-for='(item, index) in TH_gameSelectDataArr' :class="{stop: TH_stopNumberCalc[`${item.methodId}_${item.codes}`]}" :key="`${item.methodId}_${item.codes}`">
                    <div>
...
export default {
    ...
    methods: {
        ...
        async handleGameOrder() {
            if (this.TH_StopNumberFilter.length) {
                return this[_M.FILTER_NUMBER_TH]()
            }
            this[_M.GAME_ORDER_TH_SUBMIT]()
            // // 檢查整個投注箱

<script>
    //2D3D4D
    import GameQuickVN from '@C/games/GameQuickVN'
    export default {
        name: 'VN_2D_3D_4D_980_Quick',
        extends: GameQuickVN,
        data() {
            const t1 = [
                {
        ...
        methods: {
            mathResult() {
                return !this.showCodes ? '' : this.showCodes
                    ...
                    .filter(items => items && items.length === this.col[this.VN_subMenuIndex] && this.checkNum.test(items))
                    .unique()
            },
            makeCodes(original) {

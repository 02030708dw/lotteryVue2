<template>
    <div
        class="gr_games-la-header  gr_games-la-header--320  gr_container bg_secondary"
    >
        <div class="gr_games-header__inner  u_clearfix">
            <div class="gr_games-header__sectoin">
                ...
                <span class="gr_games-la-header__title">{{$t(lotteryNameKey)}}</span>
                <span
            ...
            <div class="gr_games-header__desc" v-if="!!LA_number && LA_number !== 'null'">
                <div class="gr_desc__draw">
                    #{{LA_number}}
                    <!-- 第{0}期 -->
                    <!-- {{$t("common_001", {'0': number})}} -->
                </div>
                <div class="gr_desc__time">{{LA_codeTime}}</div>
            </div>
            ...
            <div class="gr_games-header__last">
                <div class="gr_last__number">#{{LA_lastnumber}}</div>
            ...
            <div
                class="gr_games-la-lotteryStatus-warp"
                @click.stop="toggleStatus"
                :class="{is_active: isStatusActive}"
                v-if="getDataArr['LA'] && getSellTime"
            >
                <div class="gr_games-la-lotteryStatus-toggle">{{$t('timetable_003')}}</div>
                <div class="gr_games-la-lotteryStatus" :style="{'max-width': maxWStatus}" ref="lotteryStatus">
                    <div class="gr_games-la-lotteryStatus__sold">
                        <span class="gr_games-la-lotteryStatus__sold--title">
                            <!-- {{$t('贩售:')}} -->
                            {{$t('timetable_001')}}
                        </span>
                        <span class="gr_games-la-lotteryStatus__sold--txt">
                    ...
                    <div class="gr_games-la-lotteryStatus__time">
                        <span class="gr_games-la-lotteryStatus__time--title">
                            <!-- {{$t('开奖:')}} -->
                            {{$t('timetable_002')}}
                        </span>
                        <span class="gr_games-la-lotteryStatus__time--txt">
...
export default {
    ...
    computed: {
        ...
        lastballs() {
            return this.handleBallData(this.LA_lastballs)
        },
        lotteryNameKey() {
            return this.lotteryList2[this.LA_lotteryId] && this.lotteryList2[this.LA_lotteryId].title_key
        },
        isVideoGame() {
            try {
                return !!this.lotteryListVideo[this.LA_currentLottery.name]
            } catch (err) {
                return false
            }
        },
        isShowMedia() {
            const LA_currentLottery = !iframeGameSource[this.LA_lotteryId]
            this.$emit('showMedia', !LA_currentLottery)
            return !LA_currentLottery ? 0 : 2
        },
        isInFavorites() {
        ...
        // 取得販售時間資料
        getSellTime() {
            return this.getDataArr['LA'][this.$route.params.lottery]
        }
    }
}
</script>

 ...
 * 9504 投注项投注用戶限額
 */
export default class Method extends Container {
    coins = []
    codeMethod = {
        '0': this.drawTexture,
    ...
    initPrize() {
        ...
        if (Array.isArray(textData)) {
            textData.forEach(data => {
                // 非'賠率文字'的時候不塞入賠率
                if (data.isNotPrize) {
                    return new Text({ ...data, zIndex: 1, p: this })
                }
                const prize = new Text({ ...data, zIndex: 1, p: this, text })
                prize.color = data.style.fill
                this.prize.push(prize)
            })
        } else {
            const prize = new Text({ text, zIndex: 1, ...textData, p: this })

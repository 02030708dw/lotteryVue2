...
export default {
    [_M.SET_MY_TEMP_DATA]({ commit }, payload) {
        commit(_M.SET_MY_TEMP_DATA, payload)
    },
    /**
     * 投注冷卻時間
     * @param {object} that vue obj
     */
    async [_M.GET_FLIP_TIMEER_MY]({ dispatch, commit, state, rootGetters }, that) {
        if (!state.MY_flipTimer.isCountDown) {
            return
        }
        const jsDynamicConfig = state.MY_jsDynamicConfig
        if (jsDynamicConfig && jsDynamicConfig.result !== '2003') {
            const { redressTime, nowstoptime } = state.MY_flipTimer
            const nowLocalTime = +new Date()
            let countNum = state.MY_flipTimer.countNum + 1
            let nowtime = state.MY_flipTimer.nowtime + (nowLocalTime - state.MY_flipTimer.nowLocalTime)
            if (countNum % redressTime === 0) {
                commit(_M.SET_FLIP_TIMEER_MY_DATA, { isCountDown: false })
                ...
                commit(_M.SET_FLIP_TIMEER_MY_DATA, { isCountDown: true })
            }
            // const coldTime = Math.round((nowstoptime - nowtime) / 1000, 0)
            let coldTime = Math.ceil((nowstoptime - nowtime + 1100) / 1000)

            if (coldTime <= 0) {
                if (state.MY_flipTimer.isLoading) {
                    return
                }
                commit(_M.SET_FLIP_TIMEER_MY_DATA, {
                    isLoading: true,
                    coldTime: 0,
                    hours: '000',
                    minutes: '00',
                ...
                dispatch(_M.GET_GAME_JS_DYNAMIC_CONFIG_MY, that)
            }
     ...
     */
    async [_M.GET_GAME_PLAY_MY_DATA]({ state, commit, dispatch, rootGetters }, payload) {
        // console.log('開始讀取遊戲資料', +new Date() - startTime)
        const { lotteryId, subGameName, isLoadHistory } = payload
        commit(_M.SET_TEMP_DATA, { temp: 'MY_isLoading', data: true })
        commit(_M.SET_FLIP_TIMEER_MY_DATA, {
            coldTime: 0,
            isLoading: false,
        ...
        if (!gameConfig) {
            try {
                ...
                // const lotteryId = Object.values(rootGetters.lotteryList2)[10].lottery
                window.location.replace('/')
                // window.location.replace('/GamePlayerMY/' + lotteryId)
            } catch (err) {
                //redirect to '/'
                window.location.replace('/')
            }
        ...
        if (gameConfig) {
            ...
            commit(_M.SET_GAME_PLAY_MY_DATA, data)
            // 預設全選 method
            let MY_gameSubMenu = {}
            const item = rootGetters.MY_gameSubMenu[0]
            MY_gameSubMenu[item.methodId] = true
            // rootGetters.MY_gameSubMenu.forEach(({methodId}) => (MY_gameSubMenu[methodId] = true))
            commit(_M.SET_MY_TEMP_DATA, {
                temp: 'MY_methodSelect',
                data: MY_gameSubMenu
            })
            let arykey = []
            Object.values(rootGetters.MY_gameSubMenuObj).forEach((subMethod) => {
     ...
     * @param {object} that vue obj
     */
    async [_M.GET_GAME_JS_DYNAMIC_CONFIG_MY]({ state, commit, dispatch, rootGetters, rootState }, that) {
        const lotteryId = rootGetters.MY_lotteryId
        let params = { lotteryId }
        const jsDynamicConfig = await handleAjax(API.jsDynamicConfig, params, rootGetters)
        if (jsDynamicConfig) {
            if (jsDynamicConfig.data.number !== rootGetters.MY_jsDynamicConfig.number && rootState.route.name.includes('GamePlayerMY')) {
            ...
            setTimeoutFloatPrize = setTimeout(async () => {
                ...
                // 更新號碼箱內的獎金
                let data = {}
                rootGetters.MY_gameOrderArr.forEach((item) => {
                    const { methodId, codes } = item
                    const key = `${methodId}_${codes}`
                    const {hprize, autoOdds} = rootGetters.MY_jsPrizePoint[methodId][0]
                    if (!rootGetters.MY_stop[methodId]) {
    ...
    async [_M.GET_GAME_JS_DYNAMIC_CONFIG_FOR_OFFICAL_MY]({ state, commit, rootGetters }) {
        const lotteryId = rootGetters.MY_lotteryId
        // const { menuId, name } = rootGetters.lotteryList2[lotteryId] || {}
        let params = { lotteryId }
        // if ([2, 5].includes(menuId)) {
        ...
        //     }
        // }
        rootGetters.MY_jsDynamicConfig.number &&
            handleAjax(API.correctionTime, params, rootGetters, { isNotShowMessageBox: true })
                .then(({data}) => {
                    const notYetUpdated = rootGetters.MY_jsDynamicConfig.lastnumber !== data.lastnumber
                    if (notYetUpdated) {
                        const jsDynamicConfig = {
                            data: {
                                ...returnState(rootGetters.MY_jsDynamicConfig),
                                ...data
                            }
                        }
                        commit(_M.SET_GAME_PLAY_DYNAMIC_MY_DATA, jsDynamicConfig)
                    }
                }).catch(() => null)
    },
    async [_M.GAME_ORDER_MY_SUBMIT]({ state, commit, dispatch, rootGetters }, payload = {}) {
        const { instance, fast } = payload
        //需判斷有無獎期
        if (+state.MY_jsDynamicConfig.result !== 0) {
        ...
        const arr = fast ? rootGetters.MY_gameSelectDataArr : rootGetters.MY_gameOrderArr
        const getHprize = (id) => {
            if (rootGetters.MY_jsPrizePoint) {
                const { hprize, autoOdds } = rootGetters.MY_jsPrizePoint[id][0]
        ...
        // 超过投注限额
        const overLimit = arr.find(({money, methodId}) => money * getHprize(methodId) > rootGetters.MY_bonus_limit)
        const keyObj = {
            '1D': 'thai_011',
            '2D': 'thai_010',
            '3D': 'thai_009'
        ...
        if (overLimit) {
            const { title_key, parent } = rootGetters.MY_gameSubMenuObj[overLimit.methodId]
        ...
        // 需判斷號碼選擇有無完整
        // if (rootGetters.MY_gameSelectDataArr.length < 1) {
        //     dispatch(_M.SET_MESSAGE_BOX_DATA, warnMessageBox({
        //         // 号码选择不完整
        //         message: 'popup_002',
        //         // 号码选择不完整，请重新选择。
        ...
        //     return
        // }
        const initialIssue = rootGetters.MY_lastnumber
        ...
        const submit = async (instance) => {
            ...
            arr.forEach((items) => {
                let project = {
                    ...
                    nums: 1,
                    issue: state.MY_jsDynamicConfig.number,
                    lotteryId: state.MY_gameSubmit.lotteryId
                }
            ...
            const params = {
                ...
                ltMoneyAmout: fast ? rootGetters.MY_gameSelectDataMoney : rootGetters.MY_gameOrderMoney,
                orderFuture: false,
                ltProject,
                timestemp: +new Date()
            }
            ...
            if (errorCode === 0) {
                ...
                !fast && commit(_M.SET_MY_TEMP_DATA, {
                    temp: 'MY_gameOrder',
                    data: {}
                })
                commit(_M.SET_MY_TEMP_DATA, {
                ...
                dispatch(_M.SET_MESSAGE_BOX_DATA, warnMessageBox({
                    ...
                    afterConfirm: () => {
                        const isNextIssue = initialIssue !== rootGetters.MY_lastnumber
                        if (isNextIssue) return
                        // 處理停售
                        const stopMethodIds = res.data.Stop
                        commit(_M.SET_MY_TEMP_DATA, {
                            temp: 'MY_stop',
                            data: [...state.MY_stop, ...stopMethodIds].unique()
                        })
                        // 過濾注單
                        let data = {}
                        rootGetters.MY_gameOrderArr.forEach((item) => {
                            const { methodId, codes } = item
                            const key = `${methodId}_${codes}`
                            !rootGetters.MY_stop[item.methodId] && (data[key] = item)
                        })
                        commit(_M.SET_MY_TEMP_DATA, {
                            temp: 'MY_gameOrder',
                            data
                    ...
                    afterConfirm: () => {
                        ...
                        Object
                            ...
                            .forEach(([methodId, arr]) => {
                                ...
                                const {hprize: hprizeOld, autoOdds: autoOddsOld} = rootGetters.MY_jsPrizePoint[methodId][0]
                        ...
                        let data3 = {}
                        rootGetters.MY_gameSelectDataArr.forEach((item) => {
                            const { methodId, codes } = item
                            const key = `${methodId}_${codes}`
                            const {hprize, autoOdds} = rootGetters.MY_jsPrizePoint[methodId][0]
                            if (!rootGetters.MY_stop[methodId]) {

...
export const reCheckJsDynamicConfig = (jsDynamicConfig, params, num, {rootGetters, commit, dispatch}) => {
    ...
    const currentGame = rootGetters.lotteryList2[rootGetters.MY_lotteryId]
    ...
    try {
        ...
        if (diff !== 1 && diff !== -9) {
            ...
            commit(_M.SET_GAME_PLAY_DYNAMIC_MY_DATA, config)
        }
    } catch (err) {
...
export const handlePrizeData = (floatPrize, state, commit) => {
    if (floatPrize) {
        ...
        commit(_M.SET_MY_FLOAT_PRIZE, { prizeBar, keepPoint, jsPrizePoint })
    }
}
/**
 * 判斷是否有獎期
...
export const handleJsDynamicConfig = (jsDynamicConfig, params, {commit, rootGetters, dispatch}) => {
    ...
    commit(_M.SET_SUBMIT_MY_DATA, { ltIssueStart })
    commit(_M.SET_FLIP_TIMEER_MY_DATA, timeData)
...
export const updateBlockedMethods = (Stop, {state, commit, rootGetters, getters, dispatch}) => {
    ...
    commit(_M.SET_BLOCKED_MEMYODS, updatedBlockedMethods)
    ...
    dispatch(_M.SET_FLOAT_PRIZE_AND_BLOCKED_MEMYODS, newBlockeds)
}

<template>
    <div class="gr_game-my-submenu gr_game-submenu  gr_container  bg_primary  u_fz--small">
        <div class="gr_game-selectBar  u_clearfix"  >
            <template v-if="RWDMode === 1">
                <ul>
                    <li
                        v-for="({upperName_key, methodId}, cbI) in MY_gameSubMenu"
                        @click="selectMethod(!MY_methodSelect[methodId], methodId)"
                        :class="{'is_checkbox': MY_methodSelect[methodId], is_disabled: MY_stop[methodId]}"
                        :key="cbI"
                    >
        ...
        <div
            ...
            :class="{is_disabled: MY_methodIsSelect.length === 0}"
...
<script>
    import GamePrompt from '@V/GamePlayerMY/980/GamePrompt'
    ...
    export default {
        ...
        methods: {
            ...
            selectMethod(bool, methodId) {
                if (!bool && this.MY_methodIsSelect[0] === methodId) {
                    return
                }

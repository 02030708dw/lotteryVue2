...
export default {
    ...
    methods: {
        ...
        add(type, playFn = true) {
            ...
            return this.submitOrder((emptyTextAreaFn) => {
                ...
                const fn2 = () => this[_M.GAME_ORDER_VN_SUBMIT_BOX](true)

                const fn = {
                    1: emptyTextAreaFn,
                    7: () => {
                        if (isBlocked) {
                            return this.$root.$emit('blocked', { fn: fn2, fast: true, isFilter: true })
                        } else if (isStop) {
                            return this.$root.$emit('stopBet', { fn: fn2, fast: true, isFilter: true })
                        }
                        fn2()
                    }
                }
    ...
    computed: {
        ...
        maxTimes() {
            ...
            console.log('VN_isLocal',this.VN_isLocal)
            console.log('functionType',this.functionType)
            console.log('hprize',this.hprize)
            console.log('VN_limitBonus',~~(this.VN_limitBonus[key]))
            console.log('VN_limitBonus/ this.hprize',~~(this.VN_limitBonus[key] / this.hprize))

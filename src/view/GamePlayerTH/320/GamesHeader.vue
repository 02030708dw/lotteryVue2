<template>
    <div
        class="gr_games-th-header  gr_games-th-header--320  gr_container bg_secondary"
    >
        <div class="gr_games-header__inner  u_clearfix">
            <div class="gr_games-header__sectoin">
                <div class="gr_games-th-header__my-favorites  gr_my-favorites__tooltip"
                ...
                <span class="gr_games-th-header__title">{{$t(lotteryNameKey)}}</span>
                <span
            ...
            <div class="gr_games-header__desc" v-if="!!TH_number && TH_number !== 'null'">
                <div class="gr_desc__draw">
                    #{{TH_number}}
                    <!-- 第{0}期 -->
                    <!-- {{$t("common_001", {'0': number})}} -->
                </div>
                <div class="gr_desc__time">{{TH_codeTime}}</div>
            </div>
            ...
            <div class="gr_games-header__last">
                <div class="gr_last__number">#{{TH_lastnumber}}</div>
            ...
            <div
                class="gr_games-th-lotteryStatus-warp"
                @click.stop="toggleStatus"
                :class="{is_active: isStatusActive}"
                v-if="getDataArr['TH_ALL'] && getSellTime"
            >
                <div class="gr_games-th-lotteryStatus-toggle">{{$t('timetable_003')}}</div>
                <div class="gr_games-th-lotteryStatus" :style="{'max-width': maxWStatus}" ref="lotteryStatus">
                    <template v-if="lang !== 'th'">
                        <div class="gr_games-th-lotteryStatus__sold">
                            <span class="gr_games-th-lotteryStatus__sold--title">
                                <!-- {{$t('贩售:')}} -->
                                {{ $t("timetable_001") }}
                            </span>
                            <span class="gr_games-th-lotteryStatus__sold--txt">
                                {{
                        ...
                        <div class="gr_games-th-lotteryStatus__time">
                            <span class="gr_games-th-lotteryStatus__time--title">
                                <!-- {{$t('开奖:')}} -->
                                {{ $t("timetable_002") }}
                            </span>
                            <span class="gr_games-th-lotteryStatus__time--txt">
                                {{
...
export default {
    ...
    methods: {
        ...
        async setMyFavorites() {
            if (!this.isSetFavoriting) {
                ...
                if (this.isInFavorites) {
                    data = this.myFavorites.filter(id => id !== this.TH_lotteryId)
                    // '已取消最爱'
                    ...
                    data = [...this.myFavorites, this.TH_lotteryId]
                    // '已加入最爱'
        ...
        handleBallData(balls) {
            ...
            const arr = [
                ballArr[0],
                [ballArr[1], ballArr[2]].join(' '),
                [ballArr[3], ballArr[4]].join(' '),
                ballArr[5]
            ]
            return arr
        }
    ...
    computed: {
        ...
        lastballs() {
            return this.handleBallData(this.TH_lastballs)
        },
        lotteryNameKey() {
            return this.lotteryList2[this.TH_lotteryId] && this.lotteryList2[this.TH_lotteryId].title_key
        },
        isVideoGame() {
            try {
                return !!this.lotteryListVideo[this.TH_currentLottery.name]
            } catch (err) {
                return false
            }
        },
        isShowMedia() {
            const TH_currentLottery = !iframeGameSource[this.TH_lotteryId]
            this.$emit('showMedia', !TH_currentLottery)
            return !TH_currentLottery ? 0 : 2
        },
        isInFavorites() {
            return this.myFavorites.includes(this.TH_lotteryId)
        },
        isPopActive() {
        ...
        // 取得販售時間資料
        getSellTime() {
            return this.getDataArr['TH_ALL'][this.$route.params.lottery]
        }
    }
}
</script>

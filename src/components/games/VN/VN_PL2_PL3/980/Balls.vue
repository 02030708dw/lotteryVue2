    ...
    export default {
        ...
        methods: {
            selectBall(code) {
                ...
                if (bool) {
                    const data = Object.entries(this.activeData).filter(([, value]) => value)
                    if (data.length === this.VN_subMenuIndex - 1) {
                        return
                    }
                }
                this.calcOrder(code, bool)
            },
            calcOrder(code, bool) {
                this.$set(this.activeData, code, bool)
                ...
                const nums = +(data.length === this.VN_subMenuIndex - 1)

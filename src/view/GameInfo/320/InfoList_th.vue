                ...
                <tbody v-if="isShowList">
                    <tr v-for="(item, index) in tempList" :key="index">
                        <td @click="showDetail(index)">
                            ...
                            <div class="u_p--b10">
                                <span v-if="item.cnname_key === 'lottery_name_212' || item.cnname_key === 'lottery_name_223'">
                                    {{$t("popup_032")}}: {{item.issue}}
                                </span>
                                <span v-else>
                                    {{$t("popup_032")}}: {{isOfficial(item.lottery_id) ? item.belong_date.split(' ')[0] : item.issue}}
                                </span>

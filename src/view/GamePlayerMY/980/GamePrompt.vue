...
export default {
    ...
    computed: {
        ...
        tempKey() {
            const methodName = this.MY_gameMenu[this.MY_menuIndex] ? this.MY_gameMenu[this.MY_menuIndex].name : ''
            return `${methodName}@${this.lang}`
        },
        // 從MY_gameSubMenu取出需要翻譯的key
        makeKeysArr() {
            let keyArr = []
            Object.values(this.MY_gameSubMenuObj).forEach((subMethod) => {
    ...
    methods: {
        ...
        setPlayInfo() {
            ...
            if (!transArr) return
            const methodId = this.MY_methodIsSelect[0]
            const {methoddesc_key, methodexample_key} = this.MY_gameSubMenuObj[methodId].faceparamter
